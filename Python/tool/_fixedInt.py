...
#    return retValue
  def __rshift__(self, n):
      width =  self.fractWidth + self.intWidth
      fractwidth = self.fractWidth
      intwidth = self.intWidth
      roundmode = self.__roundMode
      saturatemode =  self.__saturateMode
      signedmode = self.__signedMode
      retValue = DeFixedInt(width,fractwidth,signedmode,roundmode,saturatemode)

      s = self.value & (1 << width - 1)
      self_int = self.value
      for i in range(0, n):
          self_int >>= 1
          self_int |= s
      retValue.value = self_int
      return retValue
