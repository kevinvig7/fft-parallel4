%% Punto Fijo
% Esta version considera el Producto complejo (se incrementa un bit adicional)
clc; close all; clear;
...
for clk = (0:length(datos_line1)-1)
    ...
    if(activo_min<clk && clk<activo_max) % clk: 28,...,59
        ...
        mult44Aux = bf4(2).line2 * twiddle4_l2(2,idx4); % Matlab ve la multiplicacion (0-j) como una operación full
        mult4(2).line2 = fi(mult44Aux.double,1,40,30, Fmath);
    else
        mult4(1).line2 = bf4(1).line2; 
        mult4(2).line2 = bf4(2).line2 * fi(1, St,Wt,Ft,Fmath);% Complejo*1: Resultado: s(40,30)        
    end
    % Bloque de Cuantizacion A/B 
