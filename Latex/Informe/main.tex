...
%% Subseccion
%%%%%%%%%%%%%
\subsection{Verilog (HDL) Model and optimizations} 
%\begin{figure*} 
%	\centering
...
In this subsection the different design instances to model the DFT in hardware will be described.
With the purpose to have a global view of the optimization levels to achieve the requested \textit{Timing}, four design with a working frequency of 500$MHz$ have been implemented. 
...
In order to minimize the \textit{critical path}, a second design with \textit{pipelined} registers after the quantization blocks was implemented.
In the third case of optimization the \textit{trivial} multipliers (multiplication by $-1j$) were implemented, this significantly reduced the size of the binary word.
In the fourth level of optimization showed in Fig. \ref{fig:v5esquemap}, we place inside each butterfly blocks an internal pipelined to improve the required timing, and finally, the full multipliers from stage two and five are modified to work efficiently with constant coefficients, these new multipliers are \textit{CSD} because the twiddle factors in these stages are always $-je^{-j\frac{\pi}{4}}$ and $e^{-j\frac{\pi}{4}}$.
In the fourth level of optimization showed in Fig. \ref{fig:v5esquemap}, 
a internal pipelined to each butterfly block was added in order to reduce the critical path, and finally the full multipliers from stage two and five were modified to work efficiently with constant coefficients, these new multipliers are \textit{CSD} because the twiddle factors in these stages are always $-je^{-j\frac{\pi}{4}}$ and $e^{-j\frac{\pi}{4}}$.
...
The design instances were synthesized by Synopsys tool in order to build an interconnection of Standard Cells and generate a complete set of \textit{timing-area-power report} showed in Table \ref{tab:reporte2}, \ref{tab:reporte4}, \ref{tab:reporte5} and  \ref{tab:reporte6}.

According to the first results showed in  Table \ref{tab:reporte2}, data arrival time is greater than minimum period established ($1/500Mhz=2ns$) and a time violation is detected, with this first approach we decide make a set of optimizations to folding architecture as a pipelined cutset between stages of DFT. In this way as we can see in Table \ref{tab:reporte5}, we reduced power, area and timing. 
