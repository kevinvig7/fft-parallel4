...
\begin{figure*}[t!]
	\centering
	\includegraphics[width=0.9\linewidth]{Diagramas/areaTiempoPowerArea}
...
A folding technique can reduce the number of entry points from 128 to 4 points but that result in a large amount of registers and therefore we must apply a register minimization technique. In addition,	 the number of DFT points is related with the radix-base, we can write as \textit{radix}-$8$, this representation can be expressed as \textit{radix}-$2^3$ that means we can decompose in smaller radixes with main base-2, an advantage of using a hight radix is reduced the number of multipliers.
...
%\begin{figure}[h!]
%	\centering
%	\includegraphics[width=0.75\linewidth]{Diagramas/areaTiempo}
%	\caption{}
%	\label{fig:areaTiempo}
%\end{figure}
%\begin{figure}[h!]
%	\centering
%	\includegraphics[width=0.75\linewidth]{Diagramas/powerArea}
%	\caption{}
%	\label{fig:powerArea}
%\end{figure}
