...
% Abstract
%%%%%%%%%%
\begin{abstract} - \bf In this presents a develop of a 4-parallel pipelined architecture VLSI implementation for the complex fast Fourier transform (CFFT) based on the radix-$\bf 2^3$ algorithm with 128 points using folding transformation and register minimization techniques.  A comparsion in drawn betwen th In addition, we are going to generate different synthesis levels from the hardware language description (HDL) with the purpose of obtaining a suitable performance on speed and area using an open-source FreePDK45 of 45 nm CMOS technology \cite{freePdk}. 

This work focuses on the design of 4-parallel pipelined architecture radix-$2^3$ 128-points for Complex FFT-DIF (Decimation in frequency).
