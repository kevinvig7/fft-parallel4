...
for clk = (0:length(datos_line1)-1)
    ...
    if(activo_min<clk && clk<activo_max) % clk: 28,...,59
        ...
        mult4(2).line2 = fi(mult44Aux.double,1,40,30, Fmath);
    else
        mult4(1).line2 = bf4(1).line2; 
    ...
    %% Salida Butterflys por etapa
    clk
    
    BFF(1).ck(clk+1) = bf4(1).line1;
    BFF(2).ck(clk+1) = bf4(2).line1;
    BFF(3).ck(clk+1) = bf4(1).line2;
    BFF(4).ck(clk+1) = bf4(2).line2; 
    
    %% Salida Multiplicadores por etapa 
    mult6(1).line1;
    mult6(2).line1;
    mult6(1).line2;
    mult6(2).line2;
...
if (onEscritura)
    ...
    Input128 = fopen ('C:\fft-parallel4\verilog\topfftsrcN128\v\Input128.dat','wt');
    ...
    for columna = (1:column)
        for fila = (1:row)
           ...
           % hexin=bitconcat(Re, Im);
            %fprintf(Input128,[hexin.hex '\n']);
    ...
    Output128 = fopen ('C:\fft-parallel4\verilog\topfftsrcN128\v\Output128.dat','wt');
    ...
    for columna = (1:column)
        for fila = (1:row*paralelo)
           ...
           %  hexsal=bitconcat(Re, Im);
            %fprintf(Output128,[hexsal.hex '\n']);
        end
    end
    fclose(Output128);
end
