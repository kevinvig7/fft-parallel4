%% Punto Flotante Paralelo4 - Radix8 - N:16puntos - Version Parhi
clc; close all; clear;
...
% Stage2
twiddle2_l1 = [1,1,1,1;     1,-1i,1,-1i];
% Stage3
twiddle3_l1 = [1,1,1,w2;    1,1,w4,w6];

%% Multiplicadores Line2
% Stage1
twiddle1_l2 = [1,1,1,1;     1,1,-1i,-1i];
% Stage2
twiddle2_l2 = [1,wn8,1,wn8; 1,w3n8,1,w3n8];
% Stage3
twiddle3_l2 = [1,1,w1,w3;   1,1,w5,w7];
...
% 2D1 Stage3
InReg2D1p_l2 = InReg2D;
Lat2D1p_l2   = Lat2D;
% 2D2 Stage3
InReg2D2p_l2 = InReg2D;
Lat2D2p_l2   = Lat2D;
...
for clk = (0:length(datos_line1)-1)
    ...
    if(activo_min<clk && clk<activo_max) % clk: 3,4,5,6
        ...
        % Line1 A
        [out_bf3(1)] = Mult(out_bf3(1), twiddle3_l1(1,idx), "true");
        % Line1 B
        [out_bf3(2)] = Mult(out_bf3(2), twiddle3_l1(2,idx), "true");  
    end
    out_mult3 = [out_bf3(1); out_bf3(2)];

    % Delay 2D1p
    InReg2D1p(1) = out_mult3(2);
    % Asignacion de datos_line1 al Registro
    [OutLat2D1p, OutReg2D1p] = RegIn(Lat2D1p, InReg2D1p);   
    % Dato de salida
    out_reg3 = [ out_mult3(1); OutReg2D1p];  
    ...
    % Delay 2D2p
    InReg2D2p(1) = out_switch3(1);
    % Asignacion de datos_line1 al Registro
    [OutLat2D2p, OutReg2D2p] = RegIn(Lat2D2p, InReg2D2p);   
    % Dato de salida
    out_reg3 = [ OutReg2D2p; out_switch3(2)];
    ...
    % Dato de salida
    out_reg1_l2 = [ out_mult1(2); OutReg2D1_l2];    
    ...
    % Delay 2D1p
    InReg2D1p_l2(1) = out_mult3_l2(2);
    % Asignacion de datos_line1 al Registro
    [OutLat2D1p_l2, OutReg2D1p_l2] = RegIn(Lat2D1p_l2, InReg2D1p_l2);   
    % Dato de salida
    out_reg3_l2 = [ out_mult3_l2(1); OutReg2D1p_l2];  
    
    % Divisor de Frecuencia
    % El Stage 3/LINE 2 recibe la misma señal de control que Stage3/LINE1
    [out1_l2, out2_l2] = llave(out_reg3_l2(1), out_reg3_l2(2), control3);
    out_switch3_l2 = [out1_l2; out2_l2];    
    
    % Delay 2D2p
    InReg2D2p_l2(1) = out_switch3_l2(1);
    % Asignacion de datos_line1 al Registro
    [OutLat2D2p_l2, OutReg2D2p_l2] = RegIn(Lat2D2p_l2, InReg2D2p_l2);   
    % Dato de salida
    out_reg3_l2 = [ OutReg2D2p_l2; out_switch3_l2(2)]; 
    ...
    [Lat2D1p, InReg2D1p] = RegUp(OutLat2D1p, InReg2D1p);
    [Lat2D2p, InReg2D2p] = RegUp(OutLat2D2p, InReg2D2p);
    ...
    [Lat2D1p_l2, InReg2D1p_l2] = RegUp(OutLat2D1p_l2, InReg2D1p_l2);
    [Lat2D2p_l2, InReg2D2p_l2] = RegUp(OutLat2D2p_l2, InReg2D2p_l2);
