...
# Area-Tiempo
# create a dataset
Arquitectura = c(rep(c("Imp. 1","Imp. 2","Imp. 3","Imp.4"),2))
Area=c(rep("Combinational",4),rep("Noncombinational",4))
Cantidad =c(LUT_100,FF_100)
Cantidad_num <- as.numeric(Cantidad)
data=data.frame(Area,Arquitectura,Cantidad_num,x=1:4,WNS_100)
data_frec= data.frame(x=1:4,WNS_100)

# Grouped
ggplot(data) + geom_bar(aes(fill=Area, y=Cantidad_num, x=Arquitectura),position="dodge", stat="identity") + scale_fill_manual(values=brewer.pal(n = 4, name = "Paired")) +  
...
#create a dataset
Arquitectura = c(rep(c("Imp. 1","Imp. 2","Imp. 3","Imp. 4"),2))
Power=c(rep("Total Dynamic",4),rep("Cell Leakage",4))
Cantidad =c(LUT_200,FF_200)
Cantidad_num <- as.numeric(Cantidad)
data=data.frame(Power,Arquitectura,Cantidad_num,x=1:4,WNS_200)
data_frec= data.frame(x=1:4,WNS_200)

# Grouped
ggplot(data) + geom_bar(aes(fill=Power, y=Cantidad_num, x=Arquitectura),position="dodge", stat="identity") + scale_fill_manual(values=brewer.pal(n = 4, name = "Paired")) +  
