...
function [ y ] = FFT_DIF_R2_float( x )                                            % entry point to the function of FFT decimation in frequency
p=nextpow2(length(x));                                                      % checking the size of the input array
x=[x zeros(1,(2^p)-length(x))];   %para representacion en bits ?                                          % complementing an array of zeros if necessary
N=length(x);                                                                % computing the array size
S=log2(N);                        %para saber cuantas etapas de radix necesito                                          % computing the number of conversion stages
Half=N/2;                         %Radix 2                                          % half the length of the array
